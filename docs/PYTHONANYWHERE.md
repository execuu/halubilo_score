# Alternative deployment: PythonAnywhere free pilot

The earlier online candidate was PythonAnywhere Free, with Docker on the event laptop as a manual recovery option. **Local tests do not establish online readiness.** Public HTTPS verification, the actual-host workload test, and a venue-device rehearsal must pass before event use.
...
Render Free is not suitable for local SQLite databases and uploads: its filesystem is ephemeral, free services sleep, and a free persistent disk is unavailable. See [Render's documented limitations](https://render.com/docs/free).
